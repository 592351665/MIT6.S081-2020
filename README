...
usertrap-syscall-sys_mmap，找到一个vma与文件进行映射，但此时没有实际内容；

当访问vma时会发生缺页中断，调用vmatrylazytouch函数，查找va所在的vma，找到对应的文件，从读取磁盘文件内容到物理内存页，将虚拟地址映射到物理页；

munmap部分：

usertrap-syscall-sys_munmap，取消vma文件映射，调用函数vmaunmap 将脏页写回文件，释放物理页；
